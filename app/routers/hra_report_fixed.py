...
async def knowledge_base_search(query: str, kb_id: str = "1") -> str:
    ...
    try:
        result = retrieval(kb_id, query,limit=1)
        return result
    except Exception as e:
        logger.error(f"函数调用的知识库检索失败: {e}")
        return ""
...
async def call_llm_for_interpretation(system_name: str, abnormal_indicators: List[Dict], 
    ...
    5. 涉及医疗单位时，使用中文全称（例如：毫米汞柱，而非mmHg）
    ...
    # 例如：response wa= await your_llm_api_call(prompt)
    messages = [
...
async def save_hra_interpretation_summary(db: AsyncSession, user_id: int, interpretation_summary: str):
    ...
    """
    print("save_hra_interpretation_summary")
    try:
        qa_record = QaData(
            user_id=user_id,
...
async def hra_report_interpretation(
    ...
    try:
            ...
            if system_data["状态"] == "异常" and system_data["异常指标"]:
                ...
                try:
                    interpretation = await call_llm_for_interpretation(
                     ...
                     # 验证LLM返回结果是否有效
                    if not interpretation or not isinstance(interpretation, str) or interpretation.strip() == "":
                        raise ValueError("LLM返回空结果")
                except Exception as e:
                    logger.error(f"LLM调用失败: {e}")
                    # 提供默认解读
                    abnormal_list = [f"{item['指标名称']}({item['数值']})" 
                    # abnormal_list = [f"{item['指标名称']}({format_number(item['数值'])})" 
                ...
                try:
                    ...
                    for item in system_data["异常指标"]:
                        ...
                        # abnormal_data_for_questions.append({item["指标名称"]: format_number(item["数值"])})
        ...
        print(response.code)
        # print(f"这是hra接口返回的结果{response["message"]}")
