...
#from app.database_new import SessionLocal,get_db, HraData   #导入定义的类
from app.models import  HraData,User,QaData,RadarWave,ConversationSummary
...
async def knowledge_base_chat_common(
    ...
    if not que_ans:
        try:
            ...
            # result = db.query(HraData).filter(
            #     HraData.user_id == user_id
            # ).first()

            # 使用异步查询获取HRA数据
            result = await db.execute(
                select(HraData).filter(HraData.user_id == user_id).limit(1)
            )
            ...
            #result = db.query(YourModel).filter(YourModel.user_id == user_id).first()       # new替换为 SQLAlchemy 查询
...
async def knowledge_base_chat_stream(
    ...
    return await knowledge_base_chat_common(
        user_id, kbId, query, history, stream, modelName, temperature, limit, que_ans,db
    )
...
async def knowledge_base_chat_with_hra1(
    ...
    try:
        ...
        async def event_generator(query, history, stream, modelName, temperature, kbId, limit, hra_result_str):
            ...
            # 检索逻辑
            # 检索逻辑 - 确保返回的source总是一个字典，至少包含'files'键（即使为空）
            source = retrieval(kbId=kbId, query=query, limit=limit)  or {"files:[]"}
            # logger.info(source)
